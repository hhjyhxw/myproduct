...
<script type="text/javascript">
	//删除前提示
	 function delByid(id){
	 	if(confirm("是否要删除数据？\r\n删除后，数据不可恢复！")){
	 		jump("${basePath}/${backPath}/wxImageText/delete?id="+id);
	 	}
	 }
	 //编辑
	 ...
	 function next(){
	 	jump("${basePath}/${backPath}/wxImageText/weixin_material_list?pageNo=${p.nextPage}");
	 }
	 function pre(){
	 	jump("${basePath}/${backPath}/wxImageText/weixin_material_list?pageNo=${p.prePage}");
	 }
