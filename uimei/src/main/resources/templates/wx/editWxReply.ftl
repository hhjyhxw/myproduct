...
function jump(url){
	...
	function backto_(){
	 	jump("${basePath}/${backPath}/wxReply/list?wxAppId="+document.getElementById("wxAppId").value);
	 }
					...
					<dl>
						...
						<dd class="left mL20">
							<select name="msgType" id="cph_ddlreply_form">
								<option value="1" <#if (m?? && m.msgType=='1')>selected="selected" </#if>>文本</option>
								<option value="2" <#if (m?? && m.msgType=='2')>selected="selected" </#if>>图文</option>
							</select>
						</dd>
					</dl>
					<div class="clearh"></div>
