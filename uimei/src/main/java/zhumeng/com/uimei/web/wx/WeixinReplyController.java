...
public class WeixinReplyController extends AppBaseController{
	...
	public String save(TWxReply tWxReply,String mid,String content,Long wxAppId, Model model, RedirectAttributes redirectAttributes) throws Exception {
		...
		if(list!=null && list.size()>0){
			if(tWxReply.getId()==null){
				if(tWxReply.getMsgKey().equals(list.get(0).getMsgKey())){
					...
					return "redirect:"+ConfigUtil.get("backPath")+"/wxReply/beforeAddWxReply?wxAppId="+wxAppId;
				}
			}else{
				if(tWxReply.getMsgKey().equals(list.get(0).getMsgKey())
		...
		return "redirect:"+ConfigUtil.get("backPath")+"/wxReply/list?wxAppId="+wxAppId;
	}
	...
	public String delete(Long id,Long wxAppId, RedirectAttributes redirectAttributes) throws Exception {
	...
	public String defaultWxReply(TWxReply tWxReply,Long wxAppId,Model model, RedirectAttributes redirectAttributes) throws Exception {
	...
	public String subscribeWxReply(TWxReply tWxReply,Long wxAppId,Model model, RedirectAttributes redirectAttributes) throws Exception {
	...
	public String saveSubscribeOrDefaultWxReply(TWxReply tWxReply,Long wxAppId,Model model,String content,String mid,RedirectAttributes redirectAttributes) throws Exception {
		tWxReply.setAppId(wxAppId);
		//是图文类型的回复
		if("1".equals(tWxReply.getMsgType())){
			tWxReply.setReplyMsg(content);
		...
		if("3".equals(tWxReply.getMsgEvent())){
			return "redirect:"+ConfigUtil.get("backPath")+"/wxReply/subscribeWxReply?wxAppId="+wxAppId;
		}
		//跳转默认回复页面
		if("4".equals(tWxReply.getMsgEvent())){
			return "redirect:"+ConfigUtil.get("backPath")+"/wxReply/defaultWxReply?wxAppId="+wxAppId;
		}
		return "redirect:"+ConfigUtil.get("backPath")+"/wxReply/list?wxAppId="+wxAppId;
	}
}
